...
export const TriviaGameControls: React.FC<TriviaGameControlsProps> = ({
  ...
  const getPhaseDisplay = (phase: TriviaGameState['phase']) => {
    switch (phase) {
      ...
      case 'completed':
        return { text: 'Complete', color: 'text-purple-400', bg: 'bg-purple-600' };
      default:
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-600">
              <div className="flex items-center gap-2 mb-2">
                <div className={`w-3 h-3 rounded-full ${phaseDisplay.bg}`}></div>
                <span className="text-xs text-slate-400 font-medium">STATUS</span>
              </div>
              <div className={`text-sm font-bold ${phaseDisplay.color}`}>
                {phaseDisplay.text}
              </div>
            </div>
            ...
            <div className="bg-slate-800/50 rounded-lg p-4 border border-slate-600">
              ...
              <div className="text-sm font-bold text-white">
                {gameState.timeRemaining > 0 ? `${gameState.timeRemaining}s` : `${countdownDuration}s`}
              </div>
            </div>
          ...
          <div className="space-y-3">
            {gameState.phase === 'waiting' && (
              ...
              >
                ...
                {participantCount === 0 && (
                  <span className="ml-2 text-xs">(Waiting for participants)</span>
                )}
              </Button>
            )}
          ...
          >
            ...
            >
              ...
              <p className="text-slate-300 mb-6">
                {showConfirm === 'end' && 'Are you sure you want to end the trivia question early? Participants will not be able to submit more answers.'}
                {showConfirm === 'reset' && 'This will reset the trivia question. All current responses will be cleared. Are you sure?'}
              </p>
              ...
              <div className="flex gap-3">
                ...
                <Button
                  ...
                  variant={showConfirm === 'end' ? 'danger' : 'primary'}
                  className="flex-1"
                  loading={loading}
                >
                  {showConfirm === 'end' ? 'End Question' : 'Reset'}
                </Button>
              </div>
            </motion.div>
          </motion.div>
